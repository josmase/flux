...
- [x] Migrate Reiverr, Seerr, Transmission, Gotify, and Grafana configuration PVCs to `linstor-final`; source Longhorn claims remain retained. Reiverr was pinned to worker204 after worker206 lacked the cached image; all five workloads recovered Ready.
- [x] Migrate Immich machine-learning cache to `linstor-final`; one healthy replica is active because the shared RWO cache cannot be mounted concurrently on three nodes. The previous Longhorn cache remains retained.
