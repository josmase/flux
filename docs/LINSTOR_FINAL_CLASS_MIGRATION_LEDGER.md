...
| Transmission retry | `media/transmission-config-triple` (`linstor-final-triple`) | `media/transmission-config-linstor` (`linstor`) | **Blocked**: canonical target fails CSI fsck (`Resize inode not valid; UNEXPECTED INCONSISTENCY`) on attach; no cutover | Rolled back to source; Transmission 1/1 Ready on node 206; target/snapshot retained for investigation |
| Transmission retry v2 | `media/transmission-config-triple` (`linstor-final-triple`) | `media/transmission-config-linstor-v2` (`linstor`) | **Blocked**: fresh quiesced snapshot `transmission-data-to-canonical-v2`; source/target manifest checksum `def4d9ce5d9aa6b4607ce23e78893c94f37128bd39ecefc928513d2e7f8d3737` matched, but attach-time CSI fsck again reported `Resize inode not valid; UNEXPECTED INCONSISTENCY` | Rolled back to source; Transmission 1/1 Ready on node 206; v2 target/snapshot retained |
...
| GitLab PostgreSQL | `gitlab/gitlab-postgresql-triple` (`linstor-final-triple`) | `gitlab/gitlab-postgresql-linstor` (`linstor`) | **Blocked**: quiesced snapshot restore produced source checksum `be8d199b7669f52cdbb3bbf344181cdc3a6fc2c4573287af24d840556c9e5775` vs target `41c0f4a4116cc8a4bf8b23f0a29cef3ea0f70505c7500dd43b7718332b5fd76a`; no cutover | Source restored and PostgreSQL 1/1 Ready on node 206; target/snapshot retained for investigation |
| GitLab PostgreSQL retry | `gitlab/gitlab-postgresql-triple` (`linstor-final-triple`) | `gitlab/gitlab-postgresql-linstor-v2` (`linstor`) | Graceful StatefulSet shutdown; fresh snapshot `gitlab-postgresql-to-canonical-v2`; normalized manifest checksum `584023ba6939483cd7300c997cc18e76a6d97f7207f523c62d64a27518e3b30d` matched; target has three LINSTOR resources; PostgreSQL 1/1 Ready on node 206 | Source and prior mismatched target retained; v2 target now authoritative |
| Artifactory PostgreSQL | `artifactory/artifactory-pg17-triple` (`linstor-final-triple`) | `artifactory/artifactory-pg17-linstor` (`linstor`) | Graceful StatefulSet shutdown; snapshot `artifactory-pg17-to-canonical`; normalized manifest checksum `c5bdbef28fbad582c311ba07a0f841f6a023e5304ae4172bfc93e205a56f246e` matched; target has three LINSTOR resources; PostgreSQL 1/1 Ready on node 204 | Source and snapshot retained |
| Artifactory | `artifactory/artifactory-data-triple` (`linstor-final-triple`) | `artifactory/artifactory-data-linstor` (`linstor`) | Graceful StatefulSet shutdown; snapshot `artifactory-data-to-canonical`; normalized manifest checksum `66e5ebe68153071c386e40a19b6f978e8b1d0fccb92dc02fc61353dfc2625731` matched; target has three LINSTOR resources; all 8 Artifactory containers Ready on node 206 | Source and snapshot retained |
| Immich machine-learning cache | `immich/immich-model-cache-ml-0-triple`, `-1-triple`, `-2-triple`, `-triple` (`linstor-final-triple`) | `immich/immich-model-cache-ml-0-linstor`, `-1-linstor`, `-2-linstor`, `-linstor` (`linstor`) | Deployment quiesced; four snapshots (`immich-model-cache-ml-{0,1,2,-}to-canonical`) restored to canonical PVCs; normalized file-manifest checksums were empty and matched for all four cache volumes; deployment 1/1 Ready on node 206 | Source PVCs/snapshots retained for rollback; cache is reproducible |
| Immich PostgreSQL | `immich/immich-db-triple` (`linstor-final-triple`) | `immich/immich-db-linstor` (`linstor`) | Graceful Deployment shutdown; fresh snapshot `immich-db-to-canonical-v2`; normalized data-directory checksum `bcce885f541b0978eddb46a27ce571e8f8a774e351ae573e0cc130ac4a85ed43` matched; PostgreSQL started cleanly and `immich` database contains 51 public tables | Source and snapshot retained; target now authoritative |
...
| Checkrr retry v4 | `media/checkrr-config-triple` (`linstor-final-triple`) | `media/checkrr-config-linstor-v4` (`linstor`) | **Blocked**: fresh quiesced snapshot `checkrr-config-to-canonical-v4`; source checksum `d0e9f5e28c6675be95ba4b9ebee5c32bd2852f119416a4c25ac89009a4920db5` differed from target `4719b8f5fa3d689d5b4542d9a4c2cca708862513128dcefcccc191b70bc6df40` before cutover | Rolled back immediately; Checkrr 1/1 Ready on node 206; v4 target/snapshot retained |
...
## Immich machine-learning cache migration (2026-09-23)

- The deployment was scaled to zero before snapshot creation and restored only
  after all four canonical PVCs were Bound.
- Snapshots `immich-model-cache-ml-0-to-canonical`, `-1-to-canonical`,
  `-2-to-canonical`, and `immich-model-cache-ml-to-canonical` were ready.
- All four source and target normalized file-manifest checksums were the empty
  manifest SHA-256 (`e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`),
  as expected for a regenerable cache.
- The deployment now uses `*-linstor` claims and is Ready 1/1 on node 206.
- Source PVCs and snapshots remain retained for rollback; cache contents may be
  regenerated by Immich if needed.
